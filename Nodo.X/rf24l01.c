...
void ReadregisterNum(uint8_t reg, uint8_t num, uint8_t *buf){ // LEER BUFFER REGISTRO, MAS DE UN BIT
    // Fetch the payload
    uint8_t i;
    RF24L01_CS_SetLow();
    SPI1_Exchange_Byte(RF24L01_command_R_REGISTER | reg);
    for(i=0;i<num;i++){
        buf[i] = SPI1_Exchange_Byte(0x00);
	}
    RF24L01_CS_SetHigh();
}
